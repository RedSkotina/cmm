...
#define	DecBytes(pointer, bytes) ((WCHAR*)((char*)pointer -	bytes))
...
static bool	CreateParentDirectories(LPWSTR aFileNamePlusRoot, LPWSTR aFileNameEnd)
{
	...
	for (LPWSTR lpBackSlash = aFileNameEnd; lpBackSlash>aFileNamePlusRoot; --lpBackSlash)
		...
		case L'/':
			*lpBackSlash = L'\0';
			DbgPrint(L"Checking	%s.\n", DecBytes(aFileNamePlusRoot, gWriteRootDirectoryLength));
			if (GetFileAttributes(DecBytes(aFileNamePlusRoot, gWriteRootDirectoryLength)) != INVALID_FILE_ATTRIBUTES) {
				*lpBackSlash = L'\\';
				return false;
			}
			...
			if (!CreateDirectory(DecBytes(aFileNamePlusRoot, gWriteRootDirectoryLength), NULL)) {
...
int	wmain(int argc, LPWSTR argv[])
{
	...
	//dokanOperations->ZwCreateFile = UFSCreateFile;
